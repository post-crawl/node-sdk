...
import { EXTRACT_ENDPOINT, SEARCH_AND_EXTRACT_ENDPOINT } from "../src/constants";
...
describe("Comment Filtering Support", () => {
    ...
    it("should pass comment_filter_config in extract request", async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith(
            ...
            expect.objectContaining({
                method: "POST",
                body: expect.stringContaining('"comment_filter_config":{"min_score":10,"max_depth":2}'),
            }),
        );
    });
    ...
    it("should pass comment_filter_config in searchAndExtract request", async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith(
            ...
            expect.objectContaining({
                method: "POST",
                body: expect.stringContaining('"comment_filter_config":{"tier_limits":{"0":5},"preserve_high_quality_threads":false}'),
            }),
        );
    });
});
